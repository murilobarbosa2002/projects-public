<?php
require(PATH_CLASS.'lib'.SEP.'Utilitarios.class.php');
...
class Conexao extends Utilitarios
{
    ...
    {
        if ($this->_executed == false || !empty($this->_query))
            $this->execute();
    ...
    public function update($table, $data, $skipWhere = false)
    {
        ...
        foreach ($data as $key => $val) {
            if (is_numeric($val)){
                ...
                if (strpos($val,'NOW()')||strpos($val,'DATE()')||strpos($val,'TIME()')) {
                    $valstr[] = "`$key`" . " = $val";
                }else{
