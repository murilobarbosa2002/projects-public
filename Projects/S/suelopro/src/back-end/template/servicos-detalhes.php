...
$query = $Conexao->select('legenda,image')->from(PREFIX.$_SESSION['LANG'].'servicosfotos')->where($_SESSION['LANG'].'servicos = '.$id)->fetch();
$socials_share = array(
    'f' => '<a href="https://www.facebook.com/sharer.php?u='.$service_href.'" target="_blank"><i class="fa fa-facebook"></i></a>',
    't' => '<a href="https://twitter.com/intent/tweet?url='.$service_href.'&text='.$nome.'" target="_blank"><i class="fa fa-twitter"></i></a>',
    'l' => '<a href="https://www.linkedin.com/shareArticle?mini=true&url='.$service_href.'" target="_blank"><i class="fa fa-linkedin"></i></a>',
    'g' => '<a href="https://plus.google.com/share?url='.$service_href.'" target="_blank"><i class="fa fa-google-plus"></i></a>'
); ?>
                ...
                <?php if ($Conexao->affected_rows > 0):  ?>
                    <div class="box box-white" id="imagens-produto"> 
                        <?php foreach ($query as $index => $base): 
                            ...
                            <?php if ($index==0): ?>
                                <div class="image-g">
                                    <a href="<?php echo $Conexao->generate_image_src('servicos',1500,1500,$image) ?>" data-lightbox="servico" title="<?php echo $legenda ?>"> 
                                        <?php echo $Conexao->generate_lazy_image('servicos',515,515,$image,'cortar',$legenda); ?> 
                                    </a> 
                                </div>
                            <?php endif; ?>
                            <?php if ($index==1): ?>
                            ...
                            <?php endif; ?>
                                <?php if ($index>0): ?>
                                    <a href="<?php echo $Conexao->generate_image_src('servicos',1500,1500,$image) ?>" data-lightbox="servico" title="<?php echo $legenda ?>"> 
                                        <?php echo $Conexao->generate_lazy_image('servicos',61,61,$image,'cortar',$legenda); ?>
                                    </a>
                                <?php endif; ?>
