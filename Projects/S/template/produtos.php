    ...
    if ($Conexao->affected_rows > 0): 
        ...
        if (count($url_atual)==1||in_array('page', $url_atual)):
            ...
            <main class="produtos-servicos" id="produtos-mod-3"> 
                ...
                <?php $where = ''; 
                $Conexao->select('id')->from(PREFIX.$_SESSION['LANG'].'produtos')->where('status=1'.$where)->fetch();
                ...
                $query_imagem = ',(SELECT '.$_SESSION['LANG'].'produtosfotos.image FROM '.$_SESSION['LANG'].'produtosfotos WHERE '.$_SESSION['LANG'].'produtosfotos.'.$_SESSION['LANG'].'produtos='.$_SESSION['LANG'].'produtos.id ORDER BY ABS('.$_SESSION['LANG'].'produtosfotos.ordem) ASC LIMIT 1) AS imagem';
                $query_string = 'SELECT '.$_SESSION['LANG'].'produtos.nome, '.$_SESSION['LANG'].'produtos.id, '.$_SESSION['LANG'].'produtos.url'.$query_imagem.'
                FROM '.$_SESSION['LANG'].'produtos 
                WHERE '.$_SESSION['LANG'].'produtos.status = 1 '.$where.' ORDER BY '.$_SESSION['LANG'].'produtos.nome ASC LIMIT '.$begin.','.$numero_items; 
                                    ...
                                    <div class="produto">
                                        <a href="<?php echo $urlpagination.$url; ?>"> 
                                            <div class="content"> 
                                                <?php if (!empty($imagem)): ?>
                                                    <figure class="foto"> 
                                                        <?php echo $Conexao->generate_lazy_image('produtos',466,501,$imagem,'cortar',$nome); ?>
                                                    </figure> 
                                                <?php endif; ?>
        ...
        <?php else:  
            $query = $Conexao->select('id,url,nome,descricao')->from(PREFIX.$_SESSION['LANG'].'produtos')->where('url = \''.$final_url.'\' AND status = 1')->order_by('id','ASC')->fetch_first();
            if (isset($query)) {
                extract($query,EXTR_OVERWRITE);
                require(PATH_TEMPLATE.'produtos-detalhes.php');
            }else{
